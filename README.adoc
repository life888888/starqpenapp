...
* JDK 17.0.2 (開發環境, 建議使用 adoptium 家的 Open JDK , 避免 License 問題)
...
* 如果要打包封裝產生 .dmg 檔案，需要在對應的 MacOS 平台。
* 你無法在 Linux 及 Windows 上打包產生 .dmg。

目前沒有提供 打包封裝 MacOS DMG 的設定。

如果你需要，你可能可以自己進行修改

* pom.xml 中，仿照打包 Linux deb , Linux rpm 加入打包 MacOS 的設定。
* 新增 src/main/resources/jpackage-files/starq-pen-app.macos.jpackage.settings
裡面的設定 , 需要根據 在 MacOS 平台上的 `jpackage --help` 來查出相關設定。
* src/main/resources/images 裡面的圖片 icon 部份，需要轉檔成　`.icns`

因為沒有 MacOS 的機器可以進行測試，所以就沒有設定。
